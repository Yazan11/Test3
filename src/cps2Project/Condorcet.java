 ...
 * candidates' names must set in a different class. However the information
 * about the candidates does not need to be explicitly inputted, the information
 * will be taken from the array ballots.
 * <p>
 ...
 * against all other candidates on each ballot. If a candidate is ranked above
 * the other candidate on a majority of ballots in all comparisons, then that
 * candidate wins.
 */
		...
		for (Map.Entry<String, Map<String, Integer>> outer : candVotes.entrySet()) {
			for (Map.Entry<String, Map<String, Integer>> inner : candVotes.entrySet()) {
				if (!outer.getKey().equals(inner.getKey())) {
					// no need to insert the element that has the same key in the inner map as 
					//there is no need to compare an element with itself in Condorcet
			...
			for (Map.Entry<String, Map<String, Integer>> outer : candVotes.entrySet()) {
				...
				for (Map.Entry<String, Integer> inner : outer.getValue().entrySet()) {
					...
					if (innerI == -1) { // If the inner candidate is not ranked
						if (outerI != -1) {// but the outer one is ranked,
							// increment the votes of outer against inner.
							inner.setValue(inner.getValue() + 1);
						}
					} else if (outerI < innerI) {// Lower index --> higher
													// preference
						inner.setValue(inner.getValue() + 1);
					}
				}
	...
	protected String getWinner() {
		...
		for (Map.Entry<String, Map<String, Integer>> outer : candVotes.entrySet()) {
			...
			for (Map.Entry<String, Integer> inner : outer.getValue().entrySet()) {
				...
				if (inner.getValue() < voterBallots.size()-1) {	
				isDefeated.put(outer.getKey(), true);//it is defeated 
				}
			}
		}
